<template>
  <div id="Ehoqow">
    ...
    <el-table id="table" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
      ...
      <!-- 名称 -->
      <el-table-column prop="accountName" label="账户名称" :width=itemWidth>
      </el-table-column>
      <!-- 编辑/删除 -->
      <el-table-column prop="objectId" label="编辑/删除" :width="100">
        <template slot-scope="scope">
          ...
          <el-popconfirm title="确定要删除这个账户吗？" @confirm="deleteaccount(scope.row)">
    ...
    <!-- 修改账户Dialog -->
    <el-dialog title="编辑账单账户" :visible.sync="editBillDialogVisible" width="30%" center>
      <el-form ref="billForm" :model="form" label-width="80px">
        <el-form-item label="账户名称">
          <el-input v-model="form.accountName" :placeholder="editaccountName"></el-input>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    deleteaccount(row) {
      ...
      axios.post("https://www.li-xyz.com/LifeKeeper/DeleteBillAccount", JSON.stringify(obj), {
        headers: {
          'Token': this.token,
    ...
    requestAccount() {
      ...
      axios.get("https://www.li-xyz.com/LifeKeeper/GetBillAccountData", {
        headers: {
          'Token': this.token
        }
      }).then((result) => {
    ...
    onSubmit() {
      ...
      axios.post("https://www.li-xyz.com/LifeKeeper/UpdateBillAccount", JSON.stringify(this.form), {
        headers: {
          'Token': this.token,
          "Content-Type": "application/json",
          dataType: "json",
...
#Ehoqow .el-collapse-item__header {
  border-bottom: 0px !important;
}

#Ehoqow .el-collapse-item__wrap {
  border-bottom: 0px !important;
}
...
#Ehoqow .el-collapse-item__arrow {
  background-color: white;
